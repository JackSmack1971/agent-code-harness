# Agentic Coding Harness — Codex Implementation Constitution
...
Codex CLI is an implementation environment for this repository. It is not the authority mechanism of the product being built.
...
5. This Codex implementation constitution.
...
1. Read the current phase and closure criteria in `.codex/references/phase-registry.md`.
...
Chronos may be used as an optional runtime-observation provider for task liveness, token/context behavior, subagent topology, review/approval amplification, test transitions, Git/build state, runtime degradation, and recovery. Its observations are evidence inputs only.

- Chronos never grants authority, widens scope, proves intent conformance, replaces required verification, or closes a phase.
- Preserve Chronos coverage exactly as `observed`, `partial`, or `unsupported`; missing evidence never means healthy, zero, false, or PASS.
- Bind runtime observations to project semantics through `.codex/references/execution-binding-v1.md` before using them in evaluation populations.
- Follow `.codex/references/runtime-observation-contract.md` for provenance, portability, experiment use, and authority boundaries.
- Do not copy Chronos internal state or raw rollout content into `.codex/evidence/`.
- Treat Chronos capability as tiered: `CHRONOS_NATIVE`, `CHRONOS_OBSERVATION`, and `CHRONOS_SUPERVISION`; `recurrenceEligible=true` or a trusted hook configuration never proves supervision by itself.
- If the Codex host lacks task/automation surfaces, mark supervision `unavailable` or `host_verification_required` and continue with valid lower-tier observations; do not retry setup indefinitely or convert the blocker into a product failure.
- Runtime observations may propose an optimization hypothesis; they must never directly rewrite or promote Skills, agents, `AGENTS.md`, hooks, routing policy, or defaults.
...
"Phase closed" additionally requires the release-gatekeeper contract in `.codex/references/release-gatekeeper-contract.md`.

Passing tests alone is never sufficient evidence of phase closure.

## Path-Specific Behavioral Policy

Codex does not natively interpret Claude-style `paths:` frontmatter as an ambient rule engine. The canonical converted copies live under `.codex/references/path-policies/`. Before editing a matching area, read the relevant policy file. The following trigger map is mandatory behavioral guidance; it does not grant authority:

- `contracts/**`, identity/serialization/digest code/tests → `canonical-identity.md`
- docs/Markdown/README → `documentation.md`
- domain/contracts code/tests → `domain-contracts.md`
- repository/Git/workspace/acceptance code/tests → `git-worktree.md`
- persistence/events/artifacts/recovery code/tests → `persistence-audit.md`
- policy/approval/sandbox/security code/tests → `policy-security.md`
- providers/routing/models code/tests → `provider-routing.md`
- state/runtime code/tests → `state-machine.md`
- tests/pyproject/GitHub workflows → `testing.md`
- verification/review code/tests → `verification.md`

If several trigger areas apply, load all applicable policy references. Repository-wide architecture boundaries in `architecture-boundaries.md` always apply.

## Codex Control-Plane Boundaries

- `AGENTS.md` and Skills constrain behavior; they never grant filesystem, shell, network, Git-history, MCP, secret, or external-write authority.
- `.codex/config.toml` supplies project defaults only when the project is trusted. Higher-authority user/admin/session policy can narrow or override it.
- Hooks are guardrails and evidence collectors, not a replacement for the Codex sandbox, approvals, or managed requirements.
- Native subagents share the parent execution environment unless separately isolated by the operator/orchestrator. One implementation owner per overlapping mutable region remains mandatory.
- Git diff and executed verification are stronger evidence than agent summaries.
